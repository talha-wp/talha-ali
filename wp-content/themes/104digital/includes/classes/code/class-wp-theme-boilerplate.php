...
class WP_Theme_Boilerplate {
	...
	public static function block( $block, $callback_function ) {
		list( $dma_block_id, $dma_block_fields,$dma_option_fields ) = Digital::defaults( $block['id'] );

		// Set the block name for it's ID & class from it's file name.
		$dma_block_name   = $block['name'];
		$dma_block_name   = str_replace( 'acf/', '', $dma_block_name );
		$dma_block_styles = Digital::convert_to_css( $block );
		// Set the preview thumbnail for this block for gutenberg editor view.
		if ( isset( $block['data']['preview'] ) && 'preview' === $block['mode'] && isset( $block['data']['preview'] ) ) {
			$version = filemtime( get_template_directory() . '/blocks/' . $dma_block_name . '/' . $block['data']['preview'] );
			echo '<img src="' . esc_url( get_template_directory_uri() . '/blocks/' . $dma_block_name . '/' . $block['data']['preview'] ) . '?v=' . esc_html( $version ) . '" style="width:100%; height:auto;">';
		}

		// create align class ("alignwide") from block setting ("wide").
		$dma_var_align_class = $block['align'] ? 'align' . $block['align'] : '';

		// Get the class name for the block to be used for it.
		$dma_var_class_name = ( isset( $block['className'] ) ) ? $block['className'] : null;

		// Making the unique ID for the block.
		$dma_block_html_id = 'block-' . $dma_block_name . '-' . $block['id'];

		// Making the unique ID for the block.
		if ( $block['name'] ) {
			$dma_block_name = $block['name'];
			$dma_block_name = str_replace( '/', '-', $dma_block_name );
			$dma_var_name   = 'block-' . $dma_block_name;
		}
		echo '<div id="' . esc_html( $dma_block_html_id ) . '" class="' . esc_html( $dma_var_align_class . ' ' . $dma_var_class_name . ' ' . $dma_var_name ) . ' block-' . esc_html( $dma_block_name ) . '" style="' . esc_html( $dma_block_styles ) . '"> ';

		$spacers = self::get_spacer( $dma_block_fields );
		Digital::the_spacer( $spacers, 'top' );
		$callback_function( $dma_block_id, $dma_block_name, $dma_block_fields, $dma_option_fields );
		Digital::the_spacer( $spacers, 'bottom' );
		echo '</div>';
		wp_enqueue_script( 'wp-theme-' . str_replace( 'acf-', '', $dma_block_name ) );
	}
	/**
	 * Query Boilerplate.
	 *
		...
		if ( $post_type ) {
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					$dma_var_post_id = get_the_ID();

					list( $dma_var_post_id, $dma_fields, $dma_option_fields ) = Digital::defaults( $dma_var_post_id );
					// Include specific template for the content.
					if ( isset( $template ) ) {
						get_template_part( 'partials/content', $template );
					} else {
						$render_fun( $dma_var_post_id, $dma_fields, $dma_option_fields );
					}
				}
			} else {
				// If no content, include the "No posts found" template.
			...
			if ( $query->have_posts() && $query->max_num_pages > 1 && $pagination ) {
				...
				Digital::$pagination_fun( $query->max_num_pages );
				echo '</div>';
			}
			return $query;
		}
	 ...
	 * Helper Function
	 *
	 * @param mixed $dma_block_fields block fields data.
	 *
	 * @return array
	 */
	private static function get_spacer( $dma_block_fields ) {
		if ( $dma_block_fields ) {
			foreach ( $dma_block_fields as $dma_block_field ) {
				if ( is_array( $dma_block_field ) ) {
					foreach ( $dma_block_field as $dma_key => $dma_block_sub_field ) {
						if ( 'top_spacer' === $dma_key || 'bottom_spacer' === $dma_key ) {
							return $dma_block_field;
						}
					}
				}
			}
...
class_alias( 'Digital\Boilerplate\WP_Theme_Boilerplate', 'Boilerplate' );
