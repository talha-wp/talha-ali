...
class WP_Theme_Custom extends \Boilerplate {
	...
	public static function button_text( $object, $classes = '' ) {
	if ( ! empty( $object['url'] ) ) {

		$link  = "<a href='" . esc_url( $object['url'] ) . "'";
		$link .= " title='" . esc_attr( $object['title'] ) . "'";
		$link .= " aria-label='" . esc_attr( $object['title'] ) . "'";
	...
	public static function advance_form( $form ) {
		$dma_form_provider = $form['form_provider'] ?? null;
		if ( 'gform' === $form['form_provider'] ) {
			$dma_gform = $form['gform'] ?? null;
			if ( $dma_gform ) {
				echo do_shortcode( '[gravityform id="' . $dma_gform . '" title=false description=false ajax=true]' );
			}
		} else {
			$dma_hubspot = $form['hubspot'] ?? null;
			if ( $dma_hubspot ) {
				echo html_entity_decode( $dma_hubspot );
			}
		}
		return '';
	}
	...
	public static function nav_fallback() {
		...
		if ( is_user_logged_in() ) {
			?>
			<ul>
				<li> <?php echo esc_html__( 'Go to admin area to create navigation menu', 'Digital_td' ); ?></li>
			</ul>
			<?php
		}
	}
	...
	public static function get_author_data( $post_id ) {
		...
		if ( function_exists( 'get_field' ) ) {
			$avatar = get_field( 'dma_var_author_avatar', 'user_' . $author_id );
		}
		if ( ! $avatar ) {
	...
	public static function the_attachment_image( $image_id, $thumb_size, $args = array(), $hide_backup = false ) {
		...
		if ( empty( $attachment_image ) && ! $hide_backup ) {
			...
			if ( $default_image_id ) {
				...
				echo html_entity_decode( '<img class="is-default-image" loading="lazy" src="' . Digital_DEFAULT_IMAGE . '" />' );
			}
		} else {
		...
		if ( $post_image_id ) {
			...
			if ( ! $hide_backup ) {
				...
				if ( $default_image_id ) {
					...
					$post_image = '<img class="is-default-image" loading="lazy" src="' . Digital_DEFAULT_IMAGE . '" />';
				}
			}
		}
...
class_alias( 'Digital\Custom\WP_Theme_Custom', 'Digital' );
