...
namespace DigitalAgency\Boilerplate;
...
class WP_Theme_Boilerplate {
	...
	public static function block( $block, $callback_function ) {
		list( $dgt_block_id, $dgt_block_fields,$dgt_option_fields ) = DigitalAgency::defaults( $block['id'] );

		// Set the block name for it's ID & class from it's file name.
		$dgt_block_name   = $block['name'];
		$dgt_block_name   = str_replace( 'acf/', '', $dgt_block_name );
		$dgt_block_styles = DigitalAgency::convert_to_css( $block );
		// Set the preview thumbnail for this block for gutenberg editor view.
		if ( isset( $block['data']['preview'] ) && 'preview' === $block['mode'] && isset( $block['data']['preview'] ) ) {
			$version = filemtime( get_template_directory() . '/blocks/' . $dgt_block_name . '/' . $block['data']['preview'] );
			echo '<img src="' . esc_url( get_template_directory_uri() . '/blocks/' . $dgt_block_name . '/' . $block['data']['preview'] ) . '?v=' . esc_html( $version ) . '" style="width:100%; height:auto;">';
		}

		// create align class ("alignwide") from block setting ("wide").
		$dgt_var_align_class = $block['align'] ? 'align' . $block['align'] : '';

		// Get the class name for the block to be used for it.
		$dgt_var_class_name = ( isset( $block['className'] ) ) ? $block['className'] : null;

		// Making the unique ID for the block.
		$dgt_block_html_id = 'block-' . $dgt_block_name . '-' . $block['id'];

		// Making the unique ID for the block.
		if ( $block['name'] ) {
			$dgt_block_name = $block['name'];
			$dgt_block_name = str_replace( '/', '-', $dgt_block_name );
			$dgt_var_name   = 'block-' . $dgt_block_name;
		}
		echo '<div id="' . esc_html( $dgt_block_html_id ) . '" class="' . esc_html( $dgt_var_align_class . ' ' . $dgt_var_class_name . ' ' . $dgt_var_name ) . ' block-' . esc_html( $dgt_block_name ) . '" style="' . esc_html( $dgt_block_styles ) . '"> ';

		$spacers = self::get_spacer( $dgt_block_fields );
		DigitalAgency::the_spacer( $spacers, 'top' );
		$callback_function( $dgt_block_id, $dgt_block_name, $dgt_block_fields, $dgt_option_fields );
		DigitalAgency::the_spacer( $spacers, 'bottom' );
		echo '</div>';
		wp_enqueue_script( 'wp-theme-' . str_replace( 'acf-', '', $dgt_block_name ) );
	}
	/**
	 * Query Boilerplate.
	 *
		...
		if ( $post_type ) {
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					$query->the_post();
					$dgt_var_post_id = get_the_ID();

					list( $dgt_var_post_id, $dgt_fields, $dgt_option_fields ) = DigitalAgency::defaults( $dgt_var_post_id );
					// Include specific template for the content.
					if ( isset( $template ) ) {
						get_template_part( 'partials/content', $template );
					} else {
						$render_fun( $dgt_var_post_id, $dgt_fields, $dgt_option_fields );
					}
				}
			} else {
				// If no content, include the "No posts found" template.
			...
			if ( $query->have_posts() && $query->max_num_pages > 1 && $pagination ) {
				...
				DigitalAgency::$pagination_fun( $query->max_num_pages );
				echo '</div>';
			}
			return $query;
		}
	 ...
	 * Helper Function
	 *
	 * @param mixed $dgt_block_fields block fields data.
	 *
	 * @return array
	 */
	private static function get_spacer( $dgt_block_fields ) {
		if ( $dgt_block_fields ) {
			foreach ( $dgt_block_fields as $dgt_block_field ) {
				if ( is_array( $dgt_block_field ) ) {
					foreach ( $dgt_block_field as $dgt_key => $dgt_block_sub_field ) {
						if ( 'top_spacer' === $dgt_key || 'bottom_spacer' === $dgt_key ) {
							return $dgt_block_field;
						}
					}
				}
			}
...
class_alias( 'DigitalAgency\Boilerplate\WP_Theme_Boilerplate', 'Boilerplate' );
